          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4">
              ...
              {overview.activeSprint ? (
                ...
                <p className="mt-2 text-sm text-gray-400">No active sprint</p>
              )}
            </div>
          </div>
...
function StatusSegmentBar({ buckets }: { buckets: CountBucket[] }) {
  ...
  if (total === 0) {
    return <p className="text-sm text-gray-400">No issues yet.</p>;
  }
  return (
...
function Workload({ entries }: { entries: WorkloadEntryDto[] }) {
  ...
  if (entries.length === 0) {
    return <p className="text-sm text-gray-400">No assigned work.</p>;
  }
  return (
    <div className="space-y-3">
      {entries.map((e) => (
